...
int main(int argc, const char *argv[]) {
    ...
    // Loop over all detector/descriptor combinations and log the results
    remove("mp-7.csv");
    remove("mp-8.csv");
    remove("mp-9.csv");
    std::ofstream logfile;
    logfile.open("mp-7.csv", std::ios::app);
    logfile << "Detector;Keypoints" << std::endl;
    logfile.close();
    logfile.open("mp-8.csv", std::ios::app);
    logfile << "Det-Desc;Matches" << std::endl;
    logfile.close();
    logfile.open("mp-9.csv", std::ios::app);
    logfile << "Det-Desc;Avg.Time" << std::endl;
    logfile.close();
    ...
    for (string detectorType: detectorTypeList) {
        ...
        for (string descriptorType: descriptorTypeList) {
            ...
            cout << "#0 : Running combination " << detectorType << "-" << descriptorType << endl;

            vector<DataFrame> dataBuffer; // list of data frames which are held in memory at the same time

            // declare logging variables
            vector<float> loggedTimes;
            vector<int> loggedMatches;
            vector<int> loggedKeypoints;
            ...
            for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex++) {
                ...
                loggedKeypoints.push_back(keypoints.size());
                ...
                loggedTimes.push_back((1000.0 * t / 1.0));
                ...
                if (dataBuffer.size() > 1) // wait until at least two images have been processed
                {
                    ...
                    loggedMatches.push_back(matches.size());
            ...
            // For MP.7 we only log the number of keypoints once not for every combination of detector and descriptor
            if (descriptorType == "BRISK") {
                logfile.open("mp-7.csv", std::ios::app);
                logfile << detectorType;
                for (auto itr: loggedKeypoints) {
                    logfile << ";" << itr;
                }
                logfile << std::endl;
                logfile.close();
            ...
            logfile.open("mp-8.csv", std::ios::app);
            logfile << detectorType << "-" << descriptorType;
            for (auto itr: loggedMatches) {
                logfile << ";" << itr;
            }
            logfile << std::endl;
            logfile.close();
            ...
            logfile.open("mp-9.csv", std::ios::app);
            logfile << detectorType << "-" << descriptorType;
            logfile << ";" << avgLoggedTime << std::endl; // average(loggedTimes)
            logfile.close();
